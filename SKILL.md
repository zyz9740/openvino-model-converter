...
- **Check the LAN cache first, via the `seaweedfs-lan-storage` skill.** Before touching any public source, invoke that skill to derive the cache path for this weight's origin URL (its "source-addressed" rule under "shared-cache/...") and try downloading from there. On a gigabit LAN this is 60-110 MB/s versus often-throttled public mirrors, and the path is deterministic -- any machine converting the same model computes the same cache path, so a teammate's earlier download is immediately reusable. Do not reimplement the path derivation or the HTTP transfer here -- that logic (and any future changes to it, e.g. server migration) lives entirely in `seaweedfs-lan-storage`; this skill only decides *when* to call it.
- **If not cached, download from source and populate the cache.** If weights are hosted on HuggingFace, use the hf-mirror.com mirror (read `references/hf-mirror-guide.md` for exact config); huggingface.co is often inaccessible or very slow in China. Immediately after a successful download, hand the file to `seaweedfs-lan-storage` to upload it to its derived cache path (its upload scripts skip re-uploading if the same content is already there), so the next conversion -- this machine or any other -- hits the cache instead of the public source.
- **The 1GB "ask the user first" rule now only applies to the cold-cache, public-source case.** If the file is already in the LAN cache, download it automatically regardless of size -- LAN transfer speed makes large downloads a non-issue. Only fall back to asking the user for direct-link + size + suggested location when the cache misses AND the public source file is > 1GB.
- **Network failure handling**: try sources in order -- LAN cache, then official URL / mirror sites / curl/wget alternates. If all fail, wait 1 minute and retry up to 3 times. Only after all retries are exhausted, report to the user with every method attempted.
- **Other large artifacts this skill produces** (converted IR `.bin`, demo sample images/point clouds fetched for verification, etc.) have no origin URL of their own, so they don't fit the source-addressed cache above. When they need a LAN home (Section 6's "GitHub-ready cleanup" derived-file handling), invoke `seaweedfs-lan-storage` and let it apply its content-addressed convention (hash-prefixed path under `<project_name>/...`) -- again, this skill just calls it, it does not compute the path itself.
- **Record every cache path you resolve or populate -- do not let it live only in your own head or in shell history.** A path that isn't written down defeats the whole point of a deterministic cache: the next person (or the next you, months later) has no way to know it exists and re-downloads from the public source anyway. Two places, both mandatory whenever a LAN cache path is involved (cache hit or cache-miss-then-populate):
  1. **`scripts/fetch_assets.py` itself** -- put the resolved cache URL as the first entry in that asset's `alternate_urls` (see the template's `Asset` docstring). The script is self-contained proof of where the file lives; anyone re-running it later benefits automatically, and there's no separate file to go stale.
  2. **README's "reproduce from scratch" section** -- list each cached asset with its LAN path (not just "see fetch_assets.py"), e.g. `depth_anything_v2_vitl.pth -- cached at shared-cache/huggingface.co/depth-anything/Depth-Anything-V2-Large/depth_anything_v2_vitl.pth`. This is what a human skimming the README (without opening the script) sees. It also lets someone recognize immediately if the LAN server has been decommissioned or the path convention changed since this export was written.
...
2. **Fetch large derived assets**: `python scripts/fetch_assets.py` -- explain that this downloads pretrained weights and any large test inputs from their origins, with SHA256 verification. List which files appear after this step. If any asset has a LAN cache path (see "Pretrained weights" above), list it explicitly here too -- e.g. `depth_anything_v2_vitl.pth -- cached at shared-cache/huggingface.co/depth-anything/Depth-Anything-V2-Large/depth_anything_v2_vitl.pth` -- so it's visible without opening the script.
